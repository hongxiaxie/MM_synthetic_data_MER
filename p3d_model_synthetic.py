...
class P3D(nn.Module):
    ...
    def __init__(self, block, layers, modality='RGB',
        ...
        self.layer1 = self._make_layer(block, 16, layers[0], shortcut_type) # A

        self.layer2 = self._make_layer(block, 16, layers[1], shortcut_type, stride=2) # B
        
        self.layer3 = self._make_layer(block, 16, layers[2], shortcut_type, stride=2) # C

        self.layer4 = self._make_layer(block, 16, layers[3], shortcut_type, stride=2) # A


        self.avgpool = nn.AvgPool2d(kernel_size=(5, 5), stride=1)                              # pooling layer for res5.
        self.gap = nn.AdaptiveAvgPool3d((1,1,1))
        #self.gap1 = nn.AvgPool2d(1)
        #self.final_conv = nn.Conv2d(512 * block.expansion, num_classes, kernel_size= 1)
        self.final_conv1 = nn.Conv2d(64, num_classes, kernel_size = 1) # 128 is for batch=1, otherwise will cause error
        ...
        self.input_size=(self.input_channel,1,160,160)       # input of the network original:(160,160)
    ...
    def forward(self, x):
        ...
        x = self.maxpool_2(self.layer1(x))  #  Part Res2,([1, 256, 21, 56, 56])
        #print("layer1:",x.size())
        x = self.maxpool_2(self.layer2(x))  #  Part Res3([1, 512, 10, 28, 28])
        #print("layer2",x.size())
        x = self.maxpool_2(self.layer3(x))  #  Part Res4 ([1, 1024, 5, 14, 14])
        #print("layer3:",x.size())
        sizes=x.size()
        ...
        #print("layer4:",x.size())
        #x =  self.final_conv(x)
        #print("final conv:",x.size())
        x = self.gap(x)
        #print("gap:",x.size())
        sizes = x.size()
        x = x.view(1,-1,sizes[3],sizes[4])
        ...
        #pre_fea = Parameter(torch.FloatTensor(input_channels,classes))
        #print("pre_fea:",pre_fea.size())
        #weight = Parameter(torch.FloatTensor(input_channels,hidden_layers))

        #x = x.view(x.size(0), -1)#(N,19)
        #print("after reshape output size:",x.size())#torch.Size([N, 19])
...
def P3D199(pretrained=False,modality='RGB',**kwargs):
    ...
    """
    #model = P3D(Bottleneck, [3, 8, 36, 3], modality=modality,**kwargs)#original
    model = P3D(Bottleneck, [1, 1, 1, 1], modality=modality,**kwargs)
    if pretrained==True:
        if modality=='RGB':
...
if __name__ == '__main__':
    ...
    model = P3D199(pretrained=False,num_classes=18)

    data=torch.autograd.Variable(torch.rand(1,3,91,256,256))   # if modality=='Flow', please change the 2nd dimension 3==>2
    seq_data = torch.ones([1,3,100,256,256])
    #out=model(data)
    #print (out.size())#[10,400]
    print(summary(model,seq_data))
    #model1 = models.video.r3d_18(pretrained=False)
    #print(summary(model1,seq_data))
